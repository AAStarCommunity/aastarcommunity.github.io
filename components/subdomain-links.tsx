...
const subdomains = {
  products: [
    ...
    },
    {
      ...
      description: 'A simple gas payment framework utilizing ERC4337 and EIP7702.',
    },
    {
      name: 'CometENS',
    ...
    <section id="products-and-tools" className="py-16 bg-white">
      <div className="container mx-auto px-4">
        <div className="text-center mb-12">
          <h2 className="text-3xl md:text-4xl font-semibold text-blue-700">Products and Tools</h2>
          <Separator className="w-32 mx-auto mt-4 border-blue-300" />
        </div>

        <div>
          <h3 className="text-2xl font-semibold text-blue-800 mb-6">Products</h3>
              ...
              <Link href={item.link} key={item.name} target="_blank" rel="noopener noreferrer">
                <Card className="shadow-md hover:shadow-lg transition-shadow duration-300 rounded-lg border border-gray-200 flex flex-col bg-white p-4 h-full">
                  <CardContent className="pt-2 flex-grow flex flex-col items-center">
                    <Image src={item.svg} alt={item.name} width={100} height={55} />
                    <h4 className="text-lg font-semibold text-blue-800 mt-4">{item.name}</h4>
                    <p className="text-gray-700 text-sm text-center mt-2">{item.description}</p>
                  </CardContent>
                </Card>
              </Link>
            ))}
